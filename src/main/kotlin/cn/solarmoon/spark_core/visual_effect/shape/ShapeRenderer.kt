...
class ShapeRenderer : VisualEffectRenderer() {
    ...
    override fun render(
        ...
        physLevel.world.pcoList.forEach { body ->
            if (body.collideWithGroups == 0 || body.mcTickPos == null) return@forEach
            val shape = body.collisionShape
            if (body.lastMcTickPos == null) body.lastMcTickPos = body.mcTickPos
            ...
            if (shape is CompoundCollisionShape) {
                shape.listChildren().forEach {
                    ...
                    val parentTransform =
                        body.getTransform(null).setTranslation(lerpPos).toTransformMatrix().toMatrix4f()
