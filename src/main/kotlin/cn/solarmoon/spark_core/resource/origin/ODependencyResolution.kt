 ...
 */
sealed class ODependencyResolution {
    /** 解析成功 */
    data class Success(val resource: Any) : ODependencyResolution()

    /** 解析失败 */
    data class Missing(val dependency: OResourceDependency, val reason: String) : ODependencyResolution()

    /** 版本不兼容 */
    data class VersionMismatch(val dependency: OResourceDependency, val actual: String?, val expected: String) : ODependencyResolution()
    ...
    companion object {
        /**
         * Mojang Codec for ODependencyResolution
         */
        @JvmStatic
        val CODEC: Codec<ODependencyResolution> = RecordCodecBuilder.create { instance ->
            instance.group(
         ...
         */
        @JvmStatic
        val STREAM_CODEC = object : StreamCodec<FriendlyByteBuf, ODependencyResolution> {
            override fun decode(buffer: FriendlyByteBuf): ODependencyResolution {
            ...
            override fun encode(buffer: FriendlyByteBuf, value: ODependencyResolution) {
                when (value) {
                    is Success -> {
         ...
         */
        @JvmStatic
        val LIST_CODEC: Codec<List<ODependencyResolution>> = CODEC.listOf()
        
        /**
         * Stream codec for dependency resolution lists
         */
        @JvmStatic
        val LIST_STREAM_CODEC = object : StreamCodec<FriendlyByteBuf, List<ODependencyResolution>> {
            override fun decode(buffer: FriendlyByteBuf): List<ODependencyResolution> {
                val size = buffer.readVarInt()
                val list = ArrayList<ODependencyResolution>(size)
            ...
            override fun encode(buffer: FriendlyByteBuf, value: List<ODependencyResolution>) {
