...
public class FabrikChain3D implements Serializable
{
	...
	public void addConsecutiveBone(Vec3f directionUV, float length)
	{
		...
		if (!mChain.isEmpty())
		{
			...
			addBone( new FabrikBone3D(prevBoneEnd, directionUV.normalised(), length) );
		}
		else
		{
	...
	{
		addConsecutiveHingedBone(directionUV, length, jointType, hingeRotationAxis, 180.0f, 180.0f, Vec3f.genPerpendicularVectorQuick(hingeRotationAxis));
	}
	...
	public void addConsecutiveRotorConstrainedBone(Vec3f boneDirectionUV, float boneLength, float constraintAngleDegs)
	{
		...
		FabrikBone3D bone = new FabrikBone3D(mChain.get(mChain.size()-1).getEndLocation(), boneDirectionUV.normalise(), boneLength);
