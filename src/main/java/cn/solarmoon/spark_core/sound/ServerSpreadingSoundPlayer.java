...
public class ServerSpreadingSoundPlayer implements ISpreadingSoundPlayer {
    ...
    public UUID playSpreadingSound(Level level, SoundEvent soundEvent, SoundSource soundType, Vec3 position, Vec3 speed, float pitch, float volume, int fadeIn, int fadeOut, boolean loop) {
        if (level instanceof ServerLevel serverLevel) {
            ...
            PacketDistributor.sendToPlayersNear(
                    ...
                    new SpreadingSoundPayload(uuid, soundEvent, soundType, position, speed, pitch, volume, fadeIn, fadeOut, loop)
            );
            return uuid;
        }
        else throw new IllegalStateException("method was called on a client-side level");
    ...
    @Override
    public UUID transitionSound(Level level, UUID oldSoundSource, SoundEvent newSoundEvent, SoundSource soundType, ISoundSpreader soundSpreader, int fadeIn, int fadeOut, boolean loop) {
